...
def get_args_in():
    ...
    if len(sys.argv) > 1:
        filename = sys.argv[1]
        print('retrieving data from filename user defined: %s'%filename)
    else:
    	filename = 'test.csv'
    	print('retrieving data from default filename: %s'%filename)
    print('*************************************************************')
    return filename
...
while countline <= nblines:
    ...
    FinalStruct[FileName.replace('.csv','')] [int(currentstrut[namestructs])] = currentstrut
